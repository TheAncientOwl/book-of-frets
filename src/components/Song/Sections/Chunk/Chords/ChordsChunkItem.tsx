                      ...
                      (times !== '1' || songSettings.display.chordTimesOne) && (
                        <Tooltip label={`Strum ${times} times`}>
                          <Text
                            ...
                            fontWeight='bold'
                            // [*] theme colors
                            color={theme.chunks.item.chordsPattern.chord.section.times}
                          >
                            {times}
                          </Text>
                        </Tooltip>
                ...
                <Portal>
                  {chordConfig && (
                    <PopoverContent
                      ...
                      zIndex={1}
                      backgroundColor={
                        theme.chunks.item.chordsPattern.chord.section.popover.background
                      }
                      borderColor={theme.chunks.item.chordsPattern.chord.section.popover.border}
                    >
                      <PopoverArrow
                        // [*] theme colors
                        backgroundColor={
                          theme.chunks.item.chordsPattern.chord.section.popover.arrow
                        }
                      />
                      <PopoverCloseButton
                        // [*] theme colors
                        color={theme.chunks.item.chordsPattern.chord.section.popover.closeButton}
                      />
      ...
      {props.showPattern && songSettings.display.strummingPattern && (
        <Box position='relative' mt='0.4em'>
          <Flex
            ...
            alignItems='end'
            // [*] theme colors
            color={theme.chunks.item.chordsPattern.chord.section.pattern}
          >
